...
import { getEmployeeLogsAPI } from '../../redux/employee/apiRoute';
    ...
    try {
      ...
      if (res?.statusCode === 200) {
        ...
        const arr = res?.data?.logs?.map((el) => ({
          ...
          actorId: el?.actor?.id || el?.actor_account?.id,
          target_employee: getFullName(
            el?.target_employee?.first_name,
            el?.target_employee?.middle_name,
            el?.target_employee?.last_name
          )
        }));
