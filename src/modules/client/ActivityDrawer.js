...
const ActivityClient = ({ clientId }) => {
  ...
  const getLogs = async (pageNum) => {
    ...
    try {
      ...
      let res = await getClientLogs(params);
      if (res?.statusCode === 200) {
        ...
        const arr = res?.data?.logs?.map((el) => ({
          ...
          target:
            getFullName(
              el?.target_account?.first_name,
              el?.target_account?.middle_name,
              el?.target_account?.last_name
            ) || el?.target_account?.email
        }));
