...
import { GetBillingLogs } from '../../redux/billingIds/apiRoute';
...
const ActivityDrawer = () => {
  ...
  const getLogs = async (pageNum) => {
    ...
    try {
      ...
      let res = await GetBillingLogs(params);
      if (res?.statusCode === 200) {
        ...
        const arr = res?.data?.logs?.map((el) => ({
          ...
          actorId: el?.actor_account?.id,
          target_employee: el?.previous_value?.user_name ?? el?.current_value?.user_name
        }));
